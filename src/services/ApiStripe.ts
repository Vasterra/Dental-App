...
import { IStripeCustomer } from '../types/IStripeCustomer';
import { IStripeSubscription } from '../types/IStripeSubscription';
...
class StripeManager {
  ...
  public static async STRIPE_UPDATE_PLAN(price: string, order_id: string) {
    const request = await fetch('https://evzvxzj1sf.execute-api.eu-west-1.amazonaws.com/dev/updatePlan', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
