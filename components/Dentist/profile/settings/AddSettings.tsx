...
import {AddDentistSettingsComponent, useDentistQuery} from "../../../../generated/apolloComponents";
...
const AddSettings = () => {
  ...
  const {data, loading, error, refetch} = useDentistQuery({
    variables: {
      id: Number(intId)
    },
  });
  ...
  const initialValues = {
    id: intId,
    firstName: data?.dentist?.firstName,
    lastName: data?.dentist?.lastName,
    bio: data?.dentist?.bio,
    email: data?.dentist?.email,
    website: data?.dentist?.website,
    city: data?.dentist?.city,
    street: data?.dentist?.street,
    postIndex: data?.dentist?.postIndex,
    phone: data?.dentist?.phone,
  }
  return (
    !loading ?
      <DentistSettingBlockWrapper>
        <AddDentistSettingsComponent>
          {mutation => (
            <Formik
            ...
            >
              {props => (
                <form onSubmit={props.handleSubmit}>
                  <Grid container spacing={4}>
                    ...
                    <Grid item xs={12} sm={12} lg={6}>
                      ...
                      <DentistProfileInput
                        ...
                        placeholder="Website Name"
                      />
                      <p className="input1 text-form1 space-between ">
                                    <span>
                                      <button className="button-small">{data?.dentist?.qualifications}</button>
                                      <button className="button-small">{data?.dentist?.qualifications}</button>
                                    </span>
                  ...
                  <ButtonSubmitWrapper>
                    <button className="button-big" onClick={() => {props.resetForm()}}>Cancel</button>
                    <button className="button-big" type="submit">Update Profile</button>
                  </ButtonSubmitWrapper>
                </form>
              )}
            </Formik>
