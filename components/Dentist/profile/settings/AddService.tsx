import React, {Component, useState} from 'react';
import styled from "styled-components";
import {convertCityCoords} from "../../../../utils/search/converCityCoords";
import {API} from "aws-amplify";
import {listServices} from "../../../../graphql/queries";
import {createService, deleteService} from "../../../../graphql/mutations";

const PracticeListWrapper = styled("div")`{
  display: flex;
  justify-content: center;
  align-items: center;
  flex-direction: column;
  margin-bottom: 30px;
`;

const Input = styled("input")`{
  padding: 10px;
  border-radius: 5px;
  border: 1px solid #e0e0e0;
  margin: 10px;
  width: 250px;

  &:active {
    & + span {
      width: 100%;
    }
  }
}`;

const ConfirmButton = styled("button")`{
  width: 170px;
  cursor: pointer;
  background: #fff;
  height: 37px;
  border-radius: 30px;
  border: 1px solid #0d9da6;
  display: flex;
  flex-direction: row;
  justify-content: center;
  align-items: center;
  padding: 0 20px;
  color: #000;

  &:hover {
    background: #0d9da6;
    color: #fff;
  }

  &:disabled {
    background: gray;
    border: 1px solid gray;
    color: #000000;
    cursor: default;
  }
`;

const ServiceItem = styled("button")`{
  width: 250px;
  cursor: pointer;
  background: #fff;
  border-radius: 30px;
  border: 1px solid #0d9da6;
  display: flex;
  flex-direction: row;
  justify-content: center;
  align-items: center;
  padding: 5px;
  margin: 5px;
  color: #000;

  &:hover {
    background: #0d9da6;
    color: #fff;
  }
`;

class ServiceConfig extends Component {
  state: any = {
    services: [],
    service: [],
  }

  async componentDidMount() {
    const services: any = await API.graphql({query: listServices})
    this.setState({services: services.data.listServices.items})
  }

  render() {
    const disabled = this.state.service.length === 0
    return (
      <>
        <div style={{textAlign: 'center'}}>
          Select a services your provide
        </div>
        <Input type="text" name="service" onChange={event => this.setState({service: event.target.value})}/>
        <PracticeListWrapper>
          {
            this.state.services.map((el, key) => {
              return (
                <ServiceItem key={key} onClick={async () => {
                  await API.graphql({
                    query: deleteService,
                    variables: {
                      input: {
                        id: Number(el.service.id)
                      }
                    },
                  })
                }}>{el.service.name}</ServiceItem>
              )
            })
          }
        </PracticeListWrapper>
        {
          <ConfirmButton disabled={disabled} onClick={async () => {
            await API.graphql({
              query: createService,
              variables: {
                input: {
                  name: this.state.service
                }
              },
            })
          }}>Confirm</ConfirmButton>
        }
      </>
    )
  }
}

export default ServiceConfig